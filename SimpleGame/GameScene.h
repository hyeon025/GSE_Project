...
public:
    ...
    float zoom=1.4f;
    double walk=0;
    GameScene(Renderer& renderer,Game::World& w):r(renderer),world(w){}
    Game::Vec Project(Game::Vec p) const
    {
        double dx=p.x-world.player.p.x,dy=p.y-world.player.p.y;
        return Game::Vec(width*0.5+(dx-dy)*zoom,height*0.55-(dx+dy)*0.5*zoom);
    }
    Color Shade(Color c,float light,float alpha=1) {return Color(c.r*light,c.g*light,c.b*light,c.a*alpha);}
    ...
    {
        const Color colors[]={Color(.65f,.29f,.25f),Color(.33f,.59f,.64f),Color(.59f,.48f,.63f),
            Color(.80f,.70f,.43f),Color(.57f,.61f,.61f),Color(.39f,.61f,.40f)};
        return colors[(int)a];
    }
    void Rock(float x,float y,float scale,unsigned seed,float opacity=1)
    {
        float z=zoom; zoom*=scale;
    ...
    void Tree(float x,float y,unsigned seed,float alpha)
    {
        ...
        for(int i=0;i<4;++i)Stroke(x,y,-4+i*3,0,-16+i*11,4+(i%2)*2,1.7f,bark);
    }
    void Wall(float x,float y,unsigned seed,float alpha=1)
    {
        ...
        {
            if(row==3 && col==(int)(seed%3))continue;
        ...
        Rock(x-22*zoom,y+4*zoom,.6f,seed,alpha);
    }
    void Person(Game::Vec p,Game::Ability ability,bool player,int archetype=6,int id=0)
    {
        Game::Vec s=Project(p); float x=(float)s.x,y=(float)s.y;
    ...
    {
        auto p=Project(site.p); float x=(float)p.x,y=(float)p.y;
        auto state=Game::State(world,site.key);
        Shadow(x,y,36,11);
        switch(site.kind)
        {
        case Game::Camp:
            Tri(x,y,18,-5,37,-47,66,-9,Color(.34f,.36f,.32f),Color(.52f,.53f,.45f),Color(.25f,.28f,.26f));
            Tri(x,y,37,-47,66,-9,46,-3,Color(.47f,.47f,.39f),Color(.25f,.27f,.23f),Color(.17f,.19f,.17f));
            Stroke(x,y,37,-49,37,-5,1.5f,Color(.31f,.27f,.21f));
            Stroke(x,y,37,-47,15,4,1,Color(.58f,.56f,.44f));
            Fire(x,y,state.stage>0); break;
        case Game::Wounded:
            if(state.stage==1)Person(site.p,site.ability,false,1);
            else
            {
        ...
        case Game::Herb:
            for(int i=0;i<11;++i)
            {
                ...
                if(world.time>=state.readyAt)Oval(x,y,dx-3,-h,2.8f,1.7f,Color(.65f,.65f,.52f),leaf);
            } break;
        case Game::Cache:
            ...
            for(int i=0;i<4;++i)Stroke(x,y,-10,-7-i*4,22,-13-i*4,3,Color(.34f+i*.025f,.30f+i*.02f,.23f));
            Stroke(x,y,22,-13,42,-7,2,Color(.37f,.33f,.25f));
            if(state.stage==0)Oval(x,y,3,-26,10,6,Color(.54f,.51f,.40f),Color(.30f,.31f,.27f));
            break;
        case Game::Relic:
            Wall(x-10*zoom,y,21); Rock(x+24*zoom,y+8*zoom,.8f,71);
            Stroke(x,y,-6,-26,0,-18,1,Color(.64f,.65f,.57f));
            Stroke(x,y,0,-18,6,-27,1,Color(.64f,.65f,.57f));
            if(state.stage==0)Oval(x,y,0,-19,3,5,Color(1.4f,1.7f,1.2f,.9f),Color(.5f,.6f,.5f,0));
            break;
        case Game::Shrine:
            Wall(x-22*zoom,y,7); Wall(x+22*zoom,y,9);
            Tri(x,y,-46,-35,0,-68,46,-35,Color(.29f,.33f,.32f),Color(.56f,.56f,.50f),Color(.31f,.35f,.33f));
            Tri(x,y,-30,-36,0,-56,30,-36,Color(.20f,.25f,.23f),Color(.25f,.29f,.26f),Color(.20f,.25f,.23f));
            Rock(x,y+3*zoom,1.1f,19);
            if(state.stage>0)Fire(x,y-12*zoom,true);
            break;
        }
    }
    void Render(int targetKind,int targetIndex,const std::vector<Game::Site>& sites)
    {
        ...
        struct Draw {float depth;int kind,index,tx,ty;};
        ...
        for(int y=ty-radius;y<=ty+radius;++y)for(int x=tx-radius;x<=tx+radius;++x)
        {
            ...
            if(s.x<-90 || s.x>width+90 || s.y<-50 || s.y>height+150)continue;
            if(Game::Obstacle(x,y))list.push_back({(float)s.y,0,0,x,y});
            else if(Game::Road(Game::Vec(x*32.0+16,y*32.0+16))>40)
            {
                ...
                for(int i=0;i<3;++i)
                {
                    ...
                    Stroke((float)s.x,(float)s.y,dx,0,dx-2,-3-(h%5),.65f,Color(.35f,.40f,.29f,.55f));
                }
            }
        }
        for(size_t i=0;i<sites.size();++i)list.push_back({(float)Project(sites[i].p).y,1,(int)i,0,0});
        for(size_t i=0;i<world.npcs.size();++i)
            if(Game::Distance(world.npcs[i].p,world.player.p)<1400)list.push_back({(float)Project(world.npcs[i].p).y,2,(int)i,0,0});
        ...
        if(targetKind>=0)
        {
            ...
            r.Ellipse((float)p.x,(float)p.y,26*zoom,11*zoom,Color(.83f,.76f,.48f,.03f),Color(.83f,.76f,.48f,.36f));
        }
        std::stable_sort(list.begin(),list.end(),[](const Draw& a,const Draw& b){return a.depth<b.depth;});
        for(const auto& d:list)
        {
            if(d.depth<-120 || d.depth>height+160)continue;
            if(d.kind==0)
            {
                auto p=Project(Game::Vec(d.tx*32.0+16,d.ty*32.0+16));
                unsigned h=Game::Hash(d.tx,d.ty,37);
                float alpha=(std::abs(p.x-width*.5)<36*zoom && p.y>height*.55 && p.y<height*.55+90*zoom)?.30f:1.f;
                switch((h/101)%3)
                {
                case 0:Tree((float)p.x,(float)p.y,h,alpha);break;
                case 1:Shadow((float)p.x,(float)p.y);Rock((float)p.x,(float)p.y,1.15f,h,alpha);break;
                case 2:Wall((float)p.x,(float)p.y,h,alpha);break;
                }
            }
            else if(d.kind==1)DrawSite(sites[d.index]);
            else if(d.kind==2) {const auto& n=world.npcs[d.index];Person(n.p,n.ability,false,n.archetype,n.id);}
            else Person(world.player.p,world.player.ability,true);
        }
        r.Flush();
    }
};
