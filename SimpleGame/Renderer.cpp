...
Renderer::Renderer(int width, int height) : m_Width(width), m_Height(height)
{
    ...
    m_Terrain = Program("SolidRect.vs", "Terrain.fs");
    ...
    glDisable(GL_DEPTH_TEST); m_Vertices.reserve(100000);
    RecreateTargets();
}
Renderer::~Renderer()
{
    ReleaseTargets();
    glDeleteProgram(m_Downsample); glDeleteProgram(m_Blur); glDeleteProgram(m_Composite);
    for (const auto& entry : m_Text) glDeleteTextures(1, &entry.second.id);
    glDeleteBuffers(1, &m_Buffer); glDeleteVertexArrays(1, &m_Array);
    glDeleteProgram(m_Program); glDeleteProgram(m_Terrain);
}
GLuint Renderer::Program(const char* vertexPath, const char* fragmentPath)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        for (const auto& root : {"Shaders/", "SimpleGame/Shaders/", "../SimpleGame/Shaders/"})
        {
            ...
            if (stream) { source.assign(std::istreambuf_iterator<char>(stream), std::istreambuf_iterator<char>()); break; }
        }
        if (source.empty())
        {
            ...
            if(stream)source.assign(std::istreambuf_iterator<char>(stream),std::istreambuf_iterator<char>());
        }
        if (source.empty()) { std::cerr << "Missing shader: " << paths[i] << '\n'; glDeleteProgram(program); return 0; }
        GLuint shader = glCreateShader(types[i]);
        const char* data = source.c_str(); glShaderSource(shader, 1, &data, nullptr); glCompileShader(shader);
...
void Renderer::ReleaseTargets()
{
    ...
    release(m_Scene);
    for (auto& target : m_Bloom) release(target);
    for (auto& target : m_SoftScene) release(target);
...
void Renderer::RecreateTargets()
{
    ...
    if (maxSamples >= 2)
    {
        ...
        glRenderbufferStorageMultisample(GL_RENDERBUFFER, std::min(4, maxSamples),
            GL_RGBA16F, m_Width, m_Height);
...
void Renderer::SetPostProcessing(const PostProcessingSettings& settings)
{
    ...
    m_PostSettings.vignetteEnd = limit(settings.vignetteEnd,
        m_PostSettings.vignetteStart + 0.01f, 2, 1.5f);
    ...
    m_PostSettings.edgeBlurEnd = limit(settings.edgeBlurEnd,
        m_PostSettings.edgeBlurStart + 0.01f, 2, 1.5f);
...
void Renderer::EndWorld()
{
    ...
    if (bloom) Filter(m_Scene.texture, m_Bloom, true, settings.bloomRadius);
    if (soft) Filter(m_Scene.texture, m_SoftScene, false, settings.edgeBlurRadius);
    ...
    glUniform3f(glGetUniformLocation(m_Composite, "u_Vignette"),
        settings.vignetteStrength, settings.vignetteStart, settings.vignetteEnd);
    glUniform3f(glGetUniformLocation(m_Composite, "u_EdgeBlur"),
        settings.edgeBlurStrength, settings.edgeBlurStart, settings.edgeBlurEnd);
...
void Renderer::Flush()
{ if (!m_Vertices.empty()) { Submit(m_Vertices.data(),m_Vertices.size()); m_Vertices.clear(); } }
void Renderer::Triangle(float ax,float ay,float bx,float by,float cx,float cy,Color a,Color b,Color c)
{
    m_Vertices.push_back({ax,ay,a.r,a.g,a.b,a.a,0,0});
    m_Vertices.push_back({bx,by,b.r,b.g,b.b,b.a,0,0});
    m_Vertices.push_back({cx,cy,c.r,c.g,c.b,c.a,0,0});
}
void Renderer::Rect(float x,float y,float w,float h,Color c)
{ Triangle(x,y,x+w,y,x+w,y+h,c,c,c); Triangle(x,y,x+w,y+h,x,y+h,c,c,c); }
void Renderer::Ellipse(float x,float y,float rx,float ry,Color c,Color edge)
{
    for (int i=0;i<32;++i)
    {
        float a=i*6.2831853f/32,b=(i+1)*6.2831853f/32;
        Triangle(x,y,x+std::cos(a)*rx,y+std::sin(a)*ry,x+std::cos(b)*rx,y+std::sin(b)*ry,c,edge,edge);
    }
}
void Renderer::Line(float ax,float ay,float bx,float by,float width,Color c)
{
    float dx=bx-ax,dy=by-ay,length=std::sqrt(dx*dx+dy*dy); if(length<0.01f)return;
...
{
    if(text.empty())return; Flush();
    auto key=std::make_pair(text,size); auto found=m_Text.find(key);
    if(found==m_Text.end())
    {
        // GDI rasterizes Unicode with Korean font fallback; cache the resulting textures.
        if(m_Text.size()>=256) { for(const auto& e:m_Text)glDeleteTextures(1,&e.second.id); m_Text.clear(); }
        ...
        if(!bitmap) { SelectObject(dc,oldFont); DeleteObject(font); DeleteDC(dc); return; }
        HGDIOBJ oldBitmap=SelectObject(dc,bitmap); memset(bits,0,(size_t)w*h*4);
        SetBkColor(dc,RGB(0,0,0)); SetTextColor(dc,RGB(255,255,255));
        TextOutW(dc,1,1,text.c_str(),(int)text.size()); GdiFlush();
        auto pixels=static_cast<unsigned char*>(bits);
        for(int i=0;i<w*h;++i) { pixels[i*4+3]=pixels[i*4]; pixels[i*4]=pixels[i*4+1]=pixels[i*4+2]=255; }
        TextTexture t={0,w,h}; glGenTextures(1,&t.id); glBindTexture(GL_TEXTURE_2D,t.id);
    ...
    const auto& t=found->second; float r=color.r,g=color.g,b=color.b,a=color.a,right=x+t.width,bottom=y+t.height;
    Vertex v[]={ {x,y,r,g,b,a,0,0},{right,y,r,g,b,a,1,0},{right,bottom,r,g,b,a,1,1},
        {x,y,r,g,b,a,0,0},{right,bottom,r,g,b,a,1,1},{x,bottom,r,g,b,a,0,1} };
    Submit(v,6,t.id);
}
void Renderer::Terrain(float cameraX,float cameraY,float zoom)
{
    Flush(); Vertex v[]={ {0,0},{(float)m_Width,0},{(float)m_Width,(float)m_Height}, {0,0},{(float)m_Width,(float)m_Height},{0,(float)m_Height} };
    glUseProgram(m_Terrain); glUniform2f(glGetUniformLocation(m_Terrain,"u_Viewport"),(float)m_Width,(float)m_Height);
    glUniform1i(glGetUniformLocation(m_Terrain,"u_LinearOutput"),m_RenderingHdr ? 1 : 0);
    glUniform2f(glGetUniformLocation(m_Terrain,"u_Camera"),cameraX,cameraY); glUniform1f(glGetUniformLocation(m_Terrain,"u_Zoom"),zoom);
    glBindVertexArray(m_Array); glBindBuffer(GL_ARRAY_BUFFER,m_Buffer); glBufferData(GL_ARRAY_BUFFER,sizeof(v),v,GL_STREAM_DRAW); glDrawArrays(GL_TRIANGLES,0,6);
}
